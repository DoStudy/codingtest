...
class DoBinarySearchTree(DoNode):
    ...
    def do_insert(self, do_data):
        self.do_root = self.__do_insert_data(self.do_root, do_data)
        return self.do_root is not None

    def __do_insert_data(self, do_node, do_data):
        if do_node is None:
            do_node = DoNode(do_data)

        else:
            if do_data <= do_node.do_data:
                do_node.do_left = self.__do_insert_data(do_node.do_left, do_data)
            else:
                do_node.do_left = self.__do_insert_data(do_node.do_left, do_data)
